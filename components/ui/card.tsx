...
>(({ className, variant = "default", ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn("rounded-3xl shadow-glass transition-all duration-300", variants[variant], className)}
      {...props}
    />
  )
})
...
const CardTitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3 ref={ref} className={cn("heading-card text-platinum-white", className)} {...props} />
  ),
)
...
const CardDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => (
    <p ref={ref} className={cn("text-charcoal-gray/80 text-base leading-relaxed", className)} {...props} />
  ),
)
